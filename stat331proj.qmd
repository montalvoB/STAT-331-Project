...
In order to conduct the following linear regression we took the mean happy score and average daily income for each country. This made it so we can have one value for each country in the data set. Additionally, we had to transform our explanatory variable, average daily income, with a natural log due to the shape of the plot.

#### Linear Relationship: $$ y = 23.24 + 18.62\ln(x)$$

$$e^y = e^{23.231}\cdot x^{18.623}$$

Where y-hati is the estimated happiness score out of 100 and xi is the average daily income in 2017 purchasing power parity (PPP) dollars per country.

#### Visualizations

```{r LM}
#| fig-cap: "Figure 1: Scatter Plot of Happiness Score by Avg Daily Income"
...
```{r estimates}
#| tbl-cap: "Table 1: Model Estimates for Regression"
...
According to Table 1, the intercept for the model is 38.24. This means that when a country's average daily income is \$0, their average happiness score is 38.24 out of 100. The coefficient for the logged average daily income is 18.62. For each 1% increase in a country's average daily income, their happiness score increases by an average of 18.62 points. Additionally, the p-value for the logged average daily income is nearly 0. This indicates there is extremely strong evidence that there is a positive statistical relationship between average daily income and happiness score.
