...
```{r kfold}
#| tbl-cap: "Table 3: Table of R² for K-Folds"
...
mean_r2|>
  kable(digits = 3,
        col.names = "Mean R²",
        caption = "Mean R² Value for 14 K-Folds")|>
...
The following plot displays the distribution of R² values for each of the 14 k folds.

```{r kplot}
#| fig-cap: "Figure 4: Plot of R² for 14 K-Folds"
#| fig-align: center
...
According to Table 3, the mean R² value across the 14 k folds is 67.2%. This means that across the 14 folds, average daily income explains 67.2% of the variability in happiness scores across countries. When comparing this to our original model's R² of 65.8%, these are very close. This means that our model is a relatively good fit since the R² values are close in values. One could argue that our model is underfit, likely do to the log-transformation of average daily income. Overall, our model has shown to be a solid fit in predicting happiness score from average daily income.

## Conclusion

In summary, this analysis shows a positive association between average daily income and happiness score across countries. Our linear regression showed an effective predictive performance since the k fold cross validation revealed neither extreme over or under fitting. Our findings suggest that while income may not be the sole determinant of happiness, it remains a significant predictor according to the linear regression output (t = 17.59, df = 161, p < 0.0001). This underscores the value of economic development alongside social policies for a nation's happiness. In the future, we suggest adding more control variables and checking model conditions more thoroughly to have a more accurate gauge of the results.
